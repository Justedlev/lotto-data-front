...
export function getRepeatablesAction(from: Date, to: Date, allOrStrong: AllOrStrong): (dispatch: any) => void {
    return async dispatch => {
        try {
            dispatch(setLoadingRepeatableAction(true));
            dispatch({ type: SET_REPEATABLES_DATA, payload: { isReceived: true, repeatables: [] } });
